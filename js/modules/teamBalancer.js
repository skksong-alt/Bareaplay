...
function handleTeamDrop(e, toTeamIndex) {
    ...
    if (playerIndex > -1) {
        ...
        showNotification(`${playerName} 선수가 팀 ${fromTeamIndex + 1}에서 팀 ${toTeamIndex + 1}로 이동했습니다.`);
    }
}
...
function renderResults(teams) {
    ...
    teams.forEach((team, index) => {
        ...
        team.sort((a,b) => (b.s1 || 0) - (a.s1 || 0)).forEach(player => {
            ...
            playersHtml += `<div class="player-tag flex justify-between items-center bg-white/20 p-2 rounded-lg mb-2 cursor-grab" draggable="true">
    ...
    resultContainer.querySelectorAll('.player-tag').forEach(tag => {
        const teamCard = tag.closest('.p-4');
        const teamIndex = Array.from(resultContainer.children).indexOf(teamCard);
        const playerName = tag.querySelector('.font-semibold').textContent;
        tag.addEventListener('dragstart', (e) => handlePlayerDragStart(e, playerName, teamIndex));
    });
}

function executeTeamAssignment() {
    const attendNames = attendeesTextarea.value.split('\n').map(name => name.trim()).filter(Boolean);
    if (attendNames.length === 0) { showNotification("참가자 명단을 입력해주세요.", 'error'); resetUI(); return; }
    ...
    for (let trial = 0; trial < trialCount; trial++) {
        let pool = [...known];
        // Custom shuffle is now in app.js as window.shuffleLocal
    ...
    accounting.autoFillAttendees(attendNames);
    lineupGenerator.renderTeamSelectTabs(bestTeams);
    resetUI();
}
