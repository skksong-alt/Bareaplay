...
async function generateShareableLink() {
    ...
    try {
        ...
        const meetingDate = new Date(shareDate.value);
        const days = ['일', '월', '화', '수', '목', '금', '토'];
        const dayOfWeek = days[meetingDate.getDay()];
        const linkText = `${meetingDate.getFullYear()}년 ${meetingDate.getMonth() + 1}월 ${meetingDate.getDate()}일 (${dayOfWeek}) 팀배정 및 라인업`;
