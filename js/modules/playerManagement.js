...
async function handleFormSubmit(e) {
    ...
    if (!name) {
        window.showNotification('선수 이름은 필수입니다.', 'error');
        return;
    }
    if (!id && state.playerDB[name]) {
        window.showNotification('이미 존재하는 선수 이름입니다.', 'error');
        return;
    }
    if (id && id !== name && state.playerDB[name]) {
        window.showNotification('변경하려는 이름이 이미 존재합니다.', 'error');
        return;
    }
    const newPlayerData = {
        name,
    ...
    pageElement.innerHTML = `
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 bg-white p-6 rounded-2xl shadow-lg">
                <h2 class="text-2xl font-bold mb-4">선수 목록</h2>
            ...
            <div class="lg:col-span-1 space-y-8">
                <div class="bg-white p-6 rounded-2xl shadow-lg">
                    <h2 id="player-form-title" class="text-2xl font-bold mb-4">새 선수 추가</h2>
                    <form id="player-form" class="space-y-4">
                        <input type="hidden" id="player-id">
                        <div><label for="player-name" class="block mb-2 text-sm font-medium">이름</label><input type="text" id="player-name" class="bg-gray-50 border border-gray-300 text-sm rounded-lg block w-full p-2.5" required></div>
                        <div><label for="player-pos1" class="block mb-2 text-sm font-medium">주 포지션 (쉼표로 구분)</label><input type="text" id="player-pos1" placeholder="예: FW, LW" class="bg-gray-50 border border-gray-300 text-sm rounded-lg block w-full p-2.5"></div>
