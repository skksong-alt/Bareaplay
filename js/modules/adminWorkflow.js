...
export function init({state}) {
    ...
    const update=()=>{
        ...
        const count=Object.values(state.teamLineupCache||{}).filter(l=>l?.lineups?.length===6).length;
        const mismatch=selected?.date&&selected.date!==date;
        panel.querySelector('#operator-context').textContent=`${date||'경기 날짜 미선택'} · 팀 ${state.teams?.length||0}개 · 6쿼터 라인업 ${count}팀`+
        ...
        if(state.lastPublished && state.meetingDate && state.lastPublished.date===state.meetingDate){
            const signature=JSON.stringify({date:state.meetingDate,teams:state.teams,lineups:state.teamLineupCache,names:state.teamNames});
